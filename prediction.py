...
def t_averaged_RE(g_torch, g0_torch):
    n = g_torch.shape[0]
    return torch.sqrt(((((g_torch-g_torch.mean(axis = 1).reshape((n,1)))-g0_torch)**2).mean(axis = 1)/(g0_torch**2).mean(axis = 1)).mean())


def t_median_RE(g_torch, g0_torch):
    n = g_torch.shape[0]
    return torch.sqrt(((((g_torch-g_torch.mean(axis = 1).reshape((n,1)))-g0_torch)**2).mean(axis = 1)/(g0_torch**2).mean(axis = 1)).median())

def TPR(data, g_torch_test , g0_torch=None):
    # Calculate True Positive Rate (TPR) using the risk matrix
    device1 = data.TX.device
    data.Riskmatrix()
    data.to("cpu")
    n = data.n
    M1 = g_torch_test.reshape((n,1,n))
    if g0_torch is not None:
        M0 = g0_torch.reshape((n,1,n))
    else:
        M0 = M1
    M2 = g_torch_test.reshape((n,n,1))
    R = data.R
    Numerator = ((M1>M2)*R*torch.exp(M0)).sum(axis = 2)
    Denominator = (R*torch.exp(M0)).sum(axis = 2)
    TP = Numerator/Denominator
    data.to(device1)

    return TP.to(device1)
...
def FPR(data, g_torch_test):
    ...
    data.to(device1)
    return FP.to(device1)

def prediction_ability(data,g_torch_test,g0_torch=None):
    ...
    TP = TPR(data, g_torch_test, g0_torch).sort(axis = 1).values
    FP = FPR(data, g_torch_test).sort(axis = 1).values
    n  = data.n
    auc_array = torch.zeros(n).to(device)
    for j in range(n):
        auc_array[j]=torch.trapz(y=TP[j], x=FP[j])
    C_tau = (torch.tensor(np.delete(auc_array.cpu().detach().numpy()[eff_index[:,0]],(drop_index))).to(device)[torch.tensor(T_).sort().indices]*w_*delta_t_)[:-1].sum()
